...
deg_broad= readRDS("DEG_broad.rds")
markers_broad= readRDS("broad_cluster_markers.rds")
markers_narrow= readRDS("narrow_cluster_markers.rds")
...
### Page title 
titlePanel("snRNAseq to study early-life cardiac changes in the offspring of injured fathers"),  
p("snRNAseq of mice hearts at 21 days post injury of offsprings of uninjured (Ctrl) and cryoinjured (CI) fathers"),
navbarPage( 
NULL,  
...
### Tab1.e: Differentially expressed genes 
tabPanel( 
HTML("Differentially Expressed Genes"), 
h4("Differentially Expressed Genes"), 
"In this tab, users can visualise the table that shows genes that are Differentially Exressed.", 
"For this analysis we used the Differential analysis using Libra package, which uses a pseudobulk for each cell type and then performs EdgeR-LRT.",
tags$a(href="https://www.nature.com/articles/s41467-021-25960-2", "Squair et al, 2021, Nature Communications"),  

br(),br(), 
fluidRow(
  column( 
  4, selectInput("sub_broad_cells", "Cluster Types:", choices = c("Broad Cell clusters", "Cell types with subclusters"), 
                selected = "Broad Cell clusters")), 
  column(4,selectInput("celltype", "Cell Type", choices = unique(deg_broad$cell_type),
                      selected = unique(deg_broad$cell_type)[1])),
  column(4,numericInput("log2fc","Log2 Fold change",0,min = 0,max = 10,step = 0.1,width = NULL))
...
### Tab1.e: Cluster Markers
tabPanel( 
HTML("Cluster Markers"), 
h4("Markers for each cluster"), 
"In this tab, users can visualise the genes that are highly expressed in each cluster and can act as markers for the cluster.",
br(),br(), 
fluidRow(
  column( 
    4, selectInput("sub_broad_cells_markers", "Cluster Types:", choices = c("Broad Cell clusters", "Cell types with subclusters"), 
                  selected = "Broad Cell clusters")), 
  column(4,selectInput("celltype_markers", "Cell Type", choices = unique(markers_broad$cluster),
                      selected = unique(markers_broad$cluster)[1])),
  column(4,numericInput("log2fc_markers","Log2 Fold change",0,min = 0,max = 10,step = 0.1,width = NULL))
...
p("Coppe et al, 2024.", a(href="https://doi.org/10.1101/2024.02.09.579452", "https://doi.org/10.1101/2024.02.09.579452",target="_blank")),
br(), 
