...
deg_narrow= readRDS("DEG_narrow.rds")
markers_broad= readRDS("broad_cluster_markers.rds")
markers_narrow= readRDS("narrow_cluster_markers.rds")


sc1meta$condition = factor(sc1meta$condition, levels = c("Ctrl", "CI"))
sc1meta$orig.ident = factor(sc1meta$orig.ident, levels = c("Ctrl_1","Ctrl_2", "CI_1", "CI_2"))								   
### Useful stuff 
# Colour palette 
...
rm(tmp)
shiny::validate(need(length(ggData$val[is.infinite(ggData$val)])==0, paste0("Gene '", unique(ggData$geneName[is.infinite(ggData$val)]), "' has very low expression across all the samples such that plotting bubbleplot can lead to errors, remove this gene to plot. 
                                                                            Please plot Violin/Boxplot plot to understand this gene better."))) 
# Scale if required 
...
observeEvent(input$sub_broad_cells, {
  ...
  if(input$sub_broad_cells== "Cell types with subclusters"){
    updateSelectInput(session, inputId = "celltype", label = "Select which cells to show",
                      choices = unique(deg_narrow$cell_type), selected = NULL)
  }
})
...
output$deg_table = DT::renderDataTable(server=FALSE,{
  ...
  if(input$sub_broad_cells== "Broad Cell clusters"){
    dt = deg_broad
  }else if(input$sub_broad_cells== "Cell types with subclusters"){
    dt = deg_narrow
  }

  dt_1 = dt[dt$cell_type==input$celltype,]
  
  dt_1 = dt_1[dt_1$avg_logFC>=input$log2fc|dt_1$avg_logFC<=-input$log2fc ,]
  
  dt_1 = datatable(dt_1, rownames = FALSE, extensions = "Buttons",
...
observeEvent(input$sub_broad_cells_markers, {
  if(input$sub_broad_cells_markers== "Broad Cell clusters"){
    updateSelectInput(session, inputId = "celltype_markers", label = "Select which cells to show",
                      choices = unique(markers_broad$cluster), selected = NULL)
  }
  if(input$sub_broad_cells_markers== "Cell types with subclusters"){
    updateSelectInput(session, inputId = "celltype_markers", label = "Select which cells to show",
                      choices = sort(unique(markers_narrow$cluster)), selected = NULL)
  }
})
output$markers_table = DT::renderDataTable(server=FALSE,{
  
  if(input$sub_broad_cells_markers== "Broad Cell clusters"){
    dt_marker = markers_broad
  }else if(input$sub_broad_cells_markers== "Cell types with subclusters"){
    dt_marker = markers_narrow
  }
  
  dt_1_marker = dt_marker[dt_marker$cluster==input$celltype_markers,]
  
  
  dt_1_marker = dt_1_marker[dt_1_marker$avg_log2FC>=input$log2fc_markers ,]
  dt_1_marker= dt_1_marker %>% dplyr::select(gene, avg_log2FC, p_val, p_val_adj, dplyr::everything())
  dt_1_marker = datatable(dt_1_marker, rownames = FALSE, extensions = "Buttons",
                    filter = list(position = 'top', clear = TRUE),
                    options = list(pageLength = 10, dom = "Bfrtip",
                                  buttons = c("copy", "csv", "excel", "pdf"),
                                  scrollX = TRUE))
